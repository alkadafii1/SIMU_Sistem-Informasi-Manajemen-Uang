...
# --- TAMBAHAN SOLUSI OTOMATIS: BUAT FOLDER & FILE JIKA TIDAK DITEMUKAN ---
ARTIFACTS_DIR = os.path.join(BASE_DIR, 'artifacts')
if not os.path.exists(ARTIFACTS_DIR):
    os.makedirs(ARTIFACTS_DIR, exist_ok=True)

LABEL_MAPPING_PATH = os.path.join(ARTIFACTS_DIR, 'cluster_to_label.pkl')

# Tulis ulang file pkl biner secara otomatis sebelum dibaca oleh joblib
try:
    ...
    joblib.dump(cluster_mapping_data, LABEL_MAPPING_PATH)
    print("File cluster_to_label.pkl berhasil dibuat otomatis oleh sistem!")
except Exception as e:
    print(f"Gagal menulis file otomatis: {e}")
# ------------------------------------------------------------------------

MODEL_PATH = os.path.join(ARTIFACTS_DIR, 'final_financial_health_classifier.keras')
SCALER_PATH = os.path.join(ARTIFACTS_DIR, 'financial_health_scaler.pkl')

# Memuat Artefak Model dan Scaler seperti biasa
try:
    ...
    cluster_to_label = joblib.load(LABEL_MAPPING_PATH) # <-- Sekarang dijamin sukses membaca file di atas!
...
def generate_ai_recommendation(prediction_label: str, confidence: float, ratios: dict, summary: dict):
    ...
    sisa_pendapatan_persen = ratios['disposable_income_ratio'] * 100
